...
def test_analyze_player_image_rejection_non_yankee_uniform(mock_genai_client, mocker):
    mocker.patch("ai_services.Image.open")
    
    # Gemini returns priority 3 but says it's not a Yankee uniform
    mock_response = MagicMock()
    mock_response.text = json.dumps({
        "is_yankee_uniform": False, # Non-Yankee!
        "is_official_uniform": False,
        "is_baseball_card": True,
        "is_front_of_card": True,
        "is_rectangular": True,
        "is_clean_scan": True,
        ...
        "priority_level": 3,
        "confidence": "high",
        "reasoning": "This is a Royals card, not in a Yankee uniform."
    })
    mock_genai_client.return_value = mock_response

    result = ai_services.analyze_player_image("fake.jpg", "Fasano", "key")
