...
*   **Visible Entity Description**: Relocated the site description into the header subheader (replacing the previous 'Click on a trivia card below...' copy) and removed the standalone paragraph from above the gallery. Tuned header flex styling (`margin-right`, `flex-shrink: 0`, and `white-space: nowrap`) to preserve clean alignment of navigation and score controls.
...
*   **Infrastructure Refinement:** Unified the E2E test setup by introducing `conftest.py` for shared web server management and `tests/test_config.py` for centralized configuration.
    *   **Logic Hardening:** Improved the port availability check to fail immediately if the testing port (8001) is occupied, preventing unnecessary timeouts.
    *   **Maintainability:** Eliminated hardcoded URLs across the E2E test suite by consolidating them into a shared config module.
